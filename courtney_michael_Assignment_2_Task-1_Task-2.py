...
if __name__ == "__main__":
    ...
    allCounts = allWords.aggregateByKey(0, (lambda x, y: x + y), (lambda x1, y1: x1 + y1))

    # Get the top 20,000 words in a local array in a sorted format based on frequency
    # If you want to run it on your laptio, it may a longer time for top 20k words. 
    topWords = allCounts.filter(lambda x: x[0] != '').top(20000, lambda x: x[1])
    ...
    # Since the dictionary is much smaller than all of the words, it will be broadcast:
    small_data = dictionary.collectAsMap()
    #broadcasting the dictionary to all worker nodes
    sc.broadcast(small_data)
    #Joining the dictionary and docID RDDs:
    allDictionaryWords = allWordsWithDocID.map(lambda x: (x[0], (x[1], small_data.get(x[0]),)) if x[0] in small_data.keys() else None). filter(lambda x: x != None)
    ...
    #print(len(allDictionaryWordsInEachDoc.take(1)[1]))
    ...
    zeroOrOne = allDocsAsNumpyArrays.map(lambda x: (x[0], np.where(x[1]==0, 0, 1)))
    ...
    def getPrediction (textInput, k):
        ...
        numTimes = docIDwithWikiCat.aggregateByKey(0, (lambda x, y: x + y), (lambda x1, y1: x1 + y1))
        
        # Return the top 1 of them.
        # Ask yourself: Why we are using twice top() operation here?
        return numTimes.top(k, lambda x: x[1])[0]
